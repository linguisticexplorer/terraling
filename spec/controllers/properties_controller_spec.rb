...
describe PropertiesController do
  ...
  describe "show" do
    describe "assigns" do
      it "@property should match the passed id" do
        ...
        get :show, :params => { :id => @property.id, :group_id => @property.group.id }
      ...
      it "@values should contain all values associated with the property" do
        ...
        get :show, :params => { :id => @property.id, :group_id => @group.id, :letter => "l" }
    ...
    it "@property should be found by id through current_group" do
      ...
      get :show, :params => { :id => @property.id, :group_id => @group.id }
      expect(assigns(:property)).to eq @property
    end
  end
  ...
  describe "edit" do
    it "should authorize :update on @property" do
      ...
      allow(Group).to receive_message_chain(:categories).and_return @group.categories
      get :edit, :params => { :id => @property.id, :group_id => @group.id }
    end
    ...
    describe "assigns" do
      it "the requested property to @property" do
        @property = properties(:valid)
        get :edit, :params => { :id => @property.id, :group_id => @property.group.id }
        expect(assigns(:property)).to eq @property
      end

      it "@categories should be a hash with two level members" do
        get :edit, :params => { :id => properties(:valid), :group_id => groups(:inclusive).id }
        cats = assigns(:categories)
  ...
  describe "create" do
    it "should authorize :create on the property with params" do
      ...
      allow(Property).to receive_message_chain(:new).and_return(@property)
      allow(Group).to receive_message_chain(:find).and_return(@group)
      post :create, :params => { :group_id => @group.id, :property => {'name' => 'Javanese', 'category_id' => @category.id} }
    end

    describe "with valid params" do
      def do_valid_create
        post :create, :params => { :property => {'name' => 'FROMSPACE', :description => "lots of junk", :category_id => categories(:inclusive0).id}, :group_id => groups(:inclusive).id }
      end
      ...
      it "should set the group to current group" do
        ...
        post :create, :params => { :property => {'name' => 'FROMSPACE', :description => "lots of junk", :category_id => @category.id}, :group_id => @group.id }
    ...
    describe "with invalid params" do
      def do_invalid_create
        post :create, :params => { :property => {'name' => ''}, :group_id => groups(:inclusive).id }
      end
  ...
  describe "update" do
    it "should authorize :update on the passed property" do
      ...
      allow(Property).to receive_message_chain(:find).and_return(@property)
      allow(Group).to receive_message_chain(:find).and_return(@group)
      put :update, :params => { :id => @property.id, :property => {'name' => 'ayb'}, :group_id => @group.id }
    end
    ...
    it "loads the requested property through current group" do
      ...
      put :update, :params => { :group_id => @group.id, :id => @property.id, :property => {'name' => 'eengleesh'} }
    ...
    describe "with valid params" do
      def do_valid_update_on_property(property)
        put :update, :params => { :id => property.id, :property => {'name' => 'ayb'}, :group_id => groups(:inclusive).id }
      end
      ...
      it "calls update with the passed params on the requested property" do
        ...
        put :update, :params => { :id => @property.id, :property => {'name' => new_name}, :stored_values => {:text => "foo"}, :group_id => @group.id }
      end
    ...
    describe "with invalid params" do
      def do_invalid_update
        put :update, :params => { :id => properties(:valid), :property => {'name' => ''}, :group_id => groups(:inclusive).id }
      end
  ...
  describe "destroy" do
    def do_destroy_on_property(property)
      delete :destroy, :params => { :group_id => property.group.id, :id => property.id }
    end
    ...
    it "loads the property through current group" do
      ...
      allow(Group).to receive_message_chain(:find).and_return @group
      delete :destroy, :params => { :group_id => @group.id, :id => @property.id }
    end
    ...
    it "redirects to the properties list" do
      @property = properties(:level0)
      delete :destroy, :params => { :id => @property.id, :group_id => @property.group.id }
